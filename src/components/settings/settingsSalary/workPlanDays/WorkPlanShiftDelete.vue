<template>
  <b-modal id="WorkPlanShiftDelete" scrollable centered hide-footer>
    ...
    <div class="modal-footer">
      <button
        ...
        @click.prevent="finalDeleteWorkPlansShift"
      >
        {{ $t("yes") }}
      </button>
      <button
        ...
        @click.prevent="$bvModal.hide('WorkPlanShiftDelete')"
      >
        {{ $t("no") }}
      </button>
    </div>
...
export default {
  ...
  methods: {
    ...
    async finalDeleteWorkPlansShift() {
      this.isLoading = true;
      try {
        let response = await this.workPlanShiftData.finalDeleteWorkPlansShift(
          this.language,
          this.userAuthorizeToken
        );
        if (response.data.status == STATUS.SUCCESS) {
          this.showMsg(response.data.msg, true);
          this.$bvModal.hide("WorkPlanShiftDelete");
          this.$emit("refresh");
        } else if (response.data.status == STATUS.INVALID_TOKEN) {
